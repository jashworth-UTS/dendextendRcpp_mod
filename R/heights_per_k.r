...
#' }
#' }
dendextendRcpp_heights_per_k.dendrogram <- function(tree,lowomit)
{
   # gets a dendro tree
   ...
   # ja further increase speed and efficiency by dropping out the lower [lowomit] fraction of heights
	 start = length(heights_to_cut_by)
   heights_to_cut_by = heights_to_cut_by[ 1 : round(start*(1-lowomit)) ]
	 cat(sprintf('number of heights is %i, lowomit is %f, dropping %i smallest heights\n', start,lowomit, start-length(heights_to_cut_by)))
